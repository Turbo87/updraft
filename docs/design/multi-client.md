...
- The primary's `App` remains the single source of authoritative domain state. Each client keeps its own presentation state.
...
- The primary client's map viewport supplies the OGN area of interest. Secondary clients observe the shared traffic table but do not change traffic acquisition.

Because every UI already uses the same application protocol, this feature falls out of the architecture rather than being bolted on. The same mechanism covers related features such as repeater displays and two-seat operation.
