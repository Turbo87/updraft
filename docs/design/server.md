...
- the **bulk resource routes** (tiles, GeoJSON overlays, track tail, see [runtime.md](runtime.md#resource-storage)).
...
Each request type is marked as read-only or mutating:

- Read-only queries use GET. They may be retried and never enter the replay log.
- Mutating commands use POST. They enter the replay log and pass through client permission checks.

A transport that retries commands must add a client ID and request ID. The host uses these IDs to avoid applying the same command twice. Retry tracking does not belong in the core because local callers do not need it.

Errors use stable machine-readable codes. The frontend can match these codes and show translated messages. Request and error bodies use the Rust protocol types that generate the TypeScript bindings.
...
The server exposes tiles, GeoJSON overlays, and the `?since=<seq>` track tail as normal HTTP routes, as described in [runtime.md](runtime.md#resource-storage). These routes require authentication too.
