...
- `○` Snail trail / flight trail: No historical breadcrumb behind the aircraft. (A _forward_ 5-minute flight vector is drawn instead — see FlightVector.)
...
- `◐` Task manager (build / edit): No _task_ manager, but a **Flight-Route editor** exists (`FlightRouteEditor.qml`) — add/reorder/reverse waypoints, add by double-tapping the map. It is a nav route (sequence of legs), not a scored task.
...
- `○` FLARM radar view: No dedicated relative-position radar screen. Traffic is shown on the moving map; traffic _without_ position is drawn as a proximity ring around ownship.
...
- `○` Radio frequency management / control: Frequencies are _displayed_ for airfields, but there is no radio control.
