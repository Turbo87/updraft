...
> **Framing note:** The G3X is a general-aviation glass-cockpit EFIS for experimental /
> LSA aircraft, *not* a soaring flight computer. Scored here against XCSoar's soaring
> taxonomy, most soaring-specific rows come out **absent** (MacCready, polar, thermal
> assistant, tasks/AAT, IGC, FLARM, WeGlide, live tracking), while a large body of
> powered-EFIS capability that has no row in this taxonomy is captured in §3.99.
> "Present via ADS-B/TIS/TAS" is called out explicitly wherever it stands in for a
> FLARM row, so the distinction isn't lost.
...
- **Platform(s):** Dedicated Garmin hardware — GDU 37X / GDU 4XX sunlight-readable displays, knob/joystick/softkey driven (non-touch). Experimental & Light-Sport aircraft only; explicitly *not* TSO-certified. No general-purpose OS.
...
- `●` Glide range ("reachability") line: **Glide Range Ring** — cyan reachable-area ring from pilot-entered Best Glide Speed + sink rate, corrected for AGL height, wind and bank; fades in above 500 ft AGL, with off-profile arrows. Engine-out safety tool, *not* a MacCready soaring reachability line.
...
- `○` Bluetooth: for the non-touch G3X, datalink devices connect via RS-232 serial. (The manual carries some G3X *Touch* text mentioning Bluetooth, which does not apply to this platform.)
- `○` BLE.
