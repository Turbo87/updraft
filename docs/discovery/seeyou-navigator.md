...
- **Platform(s):** iOS (16.6+) and Android phones/tablets. The identical app is the OS of Naviter's Oudie N and Omni (Android-based) dedicated devices. No PC/Mac/Linux build (that role is filled by the separate _SeeYou for PC_ and _SeeYou.Cloud_ products in the same subscription bundle).
- **License:** Proprietary / commercial. Part of the _SeeYou Subscription_ (~$9.99/month or $54.99/year). A free tier exists with reduced functionality; several features (waypoint/task collections, logbook, rain radar, sync) expect a SeeYou Cloud account.
