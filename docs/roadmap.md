...
- [x] **core-app** — the central `App` struct composed from per-domain modules, the `handle(Input) -> Update { changes, effects }` entry point, `query()`/`snapshot()`, and the `Input`/`Change`/`Effect`/`Command`/`Query` enums with serde serialization. No tokio/rayon/I-O dependencies in the core crate. _(needs: units)_
- [x] **core-time** — time as an input: monotonic timestamps stamped by adapters from a single process-wide epoch (no `Clock` trait), deterministic timer queue with earliest-deadline in `Update`, simulated-time test helpers, monotonic flight-time tracking. _(needs: core-app)_
- [x] **core-runtime** — the shared host runtime used by server and tauri alike: bounded FIFO input queue (block-on-full), effect executor (dedicated I/O thread, per-kind workers, async tasks), snapshot+changes fan-out with atomic subscribe, observable slow-subscriber drop, panic supervision, and queue-depth/handler-duration instrumentation. _(needs: core-app)_
- [x] **core-workers** — the async computation seam: compute effects carrying input snapshots, dirty-flag + one-in-flight + epoch scheduling, `JobFailed` on worker panic, persistent stateful workers (epoch bump resets state). First heavy tenants: live scoring and task optimization. _(needs: core-runtime, core-time)_

## Transports & walking skeleton

- [x] **server-protocol** — axum: REST endpoints for queries/commands + one multiplexed SSE state stream (snapshot, then changes; keep-alive enabled), speaking the core protocol. _(needs: server-scaffold, core-runtime)_
...
- [x] **frontend-protocol** — TypeScript protocol types (generated from the Rust types, committed, with a CI drift check), state-stream client with error handling and a data-age/staleness surface, Svelte stores bridging core state into components. _(needs: frontend-scaffold, server-protocol)_
...
- [ ] **replay** — replay engine feeding the core typed messages from IGC files at variable speed, bypassing the parser (byte-capture replay is a devmode tool); used for simulator mode, demo mode, and as the e2e fixture mechanism, migrating the e2e suite from scripted commands to replay fixtures. Input-log replay applies recorded inputs — external I/O and worker results alike — verbatim; recompute-and-compare runs as a CI verification mode with per-result divergence hashes. _(needs: igc-read, core-time)_
- [ ] **input-recording** — opt-in recording of the core's input sequence to `captures/`, written incrementally like the IGC log, with worker payloads in a droppable compressed sidecar stream; snapshot-seeded replay ("seed from snapshot X, replay from position N") alongside replay-from-empty. _(needs: replay)_
...
- [ ] **live-scoring** — in-flight optimization: score/achieved-distance data fields, optimal path display; runs as a stateful async worker, the compute seam's first heavy tenant (see [design/core.md](design/core.md)). _(needs: contest-optimizer, datafields-v1, core-workers)_
