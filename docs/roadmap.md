...
- [x] **core-time** — time as an input: monotonic timestamps stamped by adapters from a single process-wide epoch (no `Clock` trait) and a deterministic timer queue with the earliest deadline reported in `Update`. Scenario tests drive time by submitting clock inputs, with no wall clock. _(needs: core-app)_
