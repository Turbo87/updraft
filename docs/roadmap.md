...
- [ ] **core-state** — the central state struct, the pure `update()` entry point returning topic publishes + effect requests, a plain bounded FIFO input channel owned by the host runtime, and the `Command`/`Query`/`Event` enums with serde serialization in an `updraft_protocol` crate. _(needs: units)_
...
- [ ] **core-workers** — async computation pipeline: rate-limited stages, spawn effects executed by the host on worker threads with results posted back as input messages, dirty-flag + single-in-flight scheduling with apply-always results and per-worker-kind epoch invalidation. _(needs: core-state, core-time)_
...
- [ ] **replay** — replay engine feeding the core typed messages from IGC files at variable speed, bypassing the parser (byte-capture replay is a devmode tool); used for simulator mode, demo mode, and as the e2e fixture mechanism, migrating the e2e suite from scripted commands to replay fixtures. Input-log replay records all worker results verbatim; an opt-in verify mode re-runs workers and diffs against the recorded payloads. _(needs: igc-read, core-time)_
- [ ] **input-recording** — opt-in recording of the core's input sequence to `captures/`, written incrementally (and compressed) like the IGC log; snapshot-seeded replay ("seed from snapshot X, replay from position N") alongside replay-from-empty. _(needs: replay)_
