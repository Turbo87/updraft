...
| 6 | Offline basemap | MBTiles served over `updraft://basemap` |
...
- `updraft://basemap/{z}/{x}/{y}.pbf` from MBTiles.
- `updraft://airspace.geojson` projected on demand from core state.
...
The scheme is namespaced by role rather than by mechanism, because terrain and weather overlays are also tiles. `updraft://basemap/`, `updraft://terrain/` and `updraft://airspace.geojson` stay flat and self-describing where `updraft://tiles/` would immediately need a sub-namespace.
...
The style is assembled from the hosted `positron` basemap initially, moving to `updraft://basemap/` when offline MBTiles lands, plus the existing SDF spritesheet for icons.
