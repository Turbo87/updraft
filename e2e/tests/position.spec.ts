...
test('renders positions submitted through the simulation adapter', async ({ page, request }) => {
  ...
  let response = await request.post('/api/simulation/position', {
    data: { latitude: 50.823, longitude: 6.186, track: 45 },
  });
  ...
  response = await request.post('/api/simulation/position', {
    data: { latitude: 50.824, longitude: 6.187, track: 90 },
  });
  expect(response.status()).toBe(204);
