...
vi.mock('maplibre-gl', async (importOriginal) => {
  let original = await importOriginal<typeof import('maplibre-gl')>();
  function transformRequest(url: string) {
    if (url.startsWith('updraft:') || url.startsWith('http://updraft.localhost/')) {
      expect(url.startsWith(`${transport.origin}/`)).toBe(true);
      return { url: `generation://localhost${url.slice(transport.origin.length)}` };
    }
    return { url };
  }
  return {
    ...original,
    Map: class extends original.Map {
      constructor(options: ConstructorParameters<typeof original.Map>[0]) {
        super({ ...options, transformRequest });
      }

      override setTransformRequest() {
        return super.setTransformRequest(transformRequest);
      }
    },
  };
});
...
describe.each(['macos', 'windows'] as const)('%s resource URLs', (os) => {
  beforeEach(() => {
    mockConvertFileSrc(os);
    transport.origin = os === 'windows' ? 'http://updraft.localhost' : 'updraft://localhost';
  });
