...
/// Variance of one airspeed measurement, in `(m/s)²`, at
/// [`REFERENCE_ACCURACY`]. The value covers the airspeed error and the
/// gusts and turbulence that the estimate must not follow.
const MEASUREMENT_NOISE: f64 = 0.5;

/// The GNSS position accuracy that [`MEASUREMENT_NOISE`] applies at.
/// A worse accuracy scales the measurement variance up quadratically.
...
impl WindFilter {
    /// Folds one sample into the estimate. `interval` is the time since
    /// the previous sample, or `None` when there is no usable previous
    /// sample.
    pub fn update(
        &mut self,
        interval: Option<f64>,
        ...
        // A NaN would stay in the state for the rest of the flight, so it
        // is rejected here rather than left to the comparison.
        if air_speed.is_nan() || air_speed < MIN_AIR_SPEED {
            return;
        }
