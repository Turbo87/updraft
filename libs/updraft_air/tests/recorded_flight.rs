...
//! estimate. The estimator is still fed those samples, because it has to
//! warm up on them as it would in flight.
//!
    ...
    for line in RECORDING.lines() {
        match Record::parse_line(line) {
            ...
            Ok(Record::B(record)) => {
                ...
                let Some(state) = estimator.state().filter(|_| soaring) else {
                    continue;
                };
                vertical_speed.add(
