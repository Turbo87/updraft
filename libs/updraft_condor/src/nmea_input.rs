...
pub async fn run(listener: TcpListener, sender: broadcast::Sender<Bytes>) -> io::Result<()> {
    ...
    loop {
        ...
        current = Some(tokio::spawn(read_source(stream, peer, sender.clone())));
    }
}

async fn read_source(mut stream: TcpStream, peer: SocketAddr, sender: broadcast::Sender<Bytes>) {
    ...
    loop {
        match tokio::time::timeout(IDLE_WARNING, stream.read(&mut chunk)).await {
            ...
            Ok(Ok(count)) => {
                ...
                let output = nmea::process(&mut buffer);
                if !output.is_empty() {
