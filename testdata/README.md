...
- `condor/udp.txt`: One Condor 3 UDP datagram with both extended options, as posted by a user on the Condor forum.
- `condor/spectate.json`: A constructed Condor 3 `Spectate.json` player list in the field layout of the Condor 3 manual and the XCSoar driver tests. It contains one player without altitude and one without coordinates.
